"""Implémentation de InterfaceLLM qui appelle l'API Groq (palier gratuit)
...
ERREURS_APPEL_GROQ = (GroqError,)

# Le modèle produit parfois un JSON tronqué juste avant l'accolade finale,
# rejeté par la validation de l'API (« json_validate_failed »). C'est rare et
# transitoire : une nouvelle tentative suffit presque toujours (voir journal
# de bord du 2026-09-06). Sans ce réessai, le tour de conversation serait
# perdu en silence et le prospect devrait répéter ce qu'il vient de dire.
NOMBRE_TENTATIVES_EXTRACTION = 2


class ExtracteurGroq(InterfaceLLM):
    """Implémentation réelle de InterfaceLLM, via l'API Groq"""

    def __init__(self) -> None:
        cle_api = os.getenv("GROQ_API_KEY")
        if not cle_api or not cle_api.strip():
            raise RuntimeError("GROQ_API_KEY manquante ou vide : vérifie le fichier .env")
        self._client = Groq(api_key=cle_api)
    ...
    def extraire_besoin(self, message: str, besoin_actuel: Besoin) -> Besoin:
        ...
        for tentative in range(1, NOMBRE_TENTATIVES_EXTRACTION + 1):
            ...
            except ERREURS_APPEL_GROQ as erreur:
                logger.error(
                    "Appel Groq échoué lors de l'extraction du besoin (tentative %d/%d) : %s",
            ...
            logger.error(
                "Réponse Groq invalide lors de l'extraction du besoin (tentative %d/%d), contenu reçu : %r",
    ...
    def reformuler(self, contenu: str) -> str:
        ...
        try:
            ...
            logger.error("Appel Groq échoué lors de la reformulation : %s", erreur)
            return contenu
        ...
        if not isinstance(contenu_reformule, str) or not contenu_reformule.strip():
            logger.error(
                "Réponse Groq invalide lors de la reformulation, contenu reçu : %r", contenu_reformule
            )
            return contenu
        if contient_un_nombre_invente(contenu, contenu_reformule):
            logger.error(
                "Reformulation Groq rejetée, nombre inventé absent de la source : %r -> %r",

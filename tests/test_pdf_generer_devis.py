...
def test_mention_non_contractuelle_toujours_presente():
    texte = texte_du_pdf(generer_pdf_devis(resultat_mariage_300(), TENANT, BESOIN_MARIAGE_300))
...
def test_coordonnees_du_tenant_apparaissent_dans_len_tete():
    ...
    texte = texte_du_pdf(
        generer_pdf_devis(resultat_mariage_300(), tenant_avec_coordonnees, BESOIN_MARIAGE_300)
    )

    assert "671234567" in texte


def test_coordonnees_du_tenant_precedent_la_date_dans_len_tete():
    """Ordre demandé : nom du tenant, ses coordonnées, puis la date/heure
    d'établissement de l'estimation — dans cet ordre, pas un autre."""
    tenant_avec_coordonnees = TenantContexte(
        id="t1", nom="Événements Étoile", slug="etoile", logo=None,
        coordonnees="671234567, Bastos",
    )
    ...
    assert texte.index("671234567") < texte.index("Estimation établie le")
...
def test_aucune_information_sur_le_demandeur_naparait_dans_le_pdf():
    """Le devis est adressé au prospect, pas à propos de lui : contrairement à
    une version antérieure, aucun bloc « Demandé par » n'y figure (voir D38)."""
    texte = texte_du_pdf(generer_pdf_devis(resultat_mariage_300(), TENANT, BESOIN_MARIAGE_300))

    assert "DEMANDÉ PAR" not in texte

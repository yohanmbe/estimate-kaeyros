...
def test_repartition_par_tranche_invites_sur_quatre_demandes_et_une_sans_besoin(session):
    etoile = creer_tenant(session, "etoile")
    creer_demande(session, etoile, datetime(2026, 1, 5), besoin={"nombre_invites": 30})
    creer_demande(session, etoile, datetime(2026, 1, 6), besoin={"nombre_invites": 80})
    creer_demande(session, etoile, datetime(2026, 1, 7), besoin={"nombre_invites": 200})
    creer_demande(session, etoile, datetime(2026, 1, 8), besoin={"nombre_invites": 350})
    ...
    creer_demande(session, etoile, datetime(2026, 2, 1), besoin={"nombre_invites": 30})

    assert repartir_par_tranche_invites(session, etoile.id, PERIODE_JANVIER) == [
        EffectifTranche(tranche="1-50", nombre_demandes=1),
        EffectifTranche(tranche="51-150", nombre_demandes=1),
        EffectifTranche(tranche="151-300", nombre_demandes=1),
        EffectifTranche(tranche="301+", nombre_demandes=1),
    ]


def test_repartition_par_tranche_invites_bornes_incluses(session):
    etoile = creer_tenant(session, "etoile")
    creer_demande(session, etoile, datetime(2026, 1, 5), besoin={"nombre_invites": 50})
    creer_demande(session, etoile, datetime(2026, 1, 6), besoin={"nombre_invites": 51})
    creer_demande(session, etoile, datetime(2026, 1, 7), besoin={"nombre_invites": 300})
    creer_demande(session, etoile, datetime(2026, 1, 8), besoin={"nombre_invites": 301})
...
def test_repartition_ignore_les_demandes_dun_autre_tenant(session):
    etoile = creer_tenant(session, "etoile")
    fanta = creer_tenant(session, "fanta")
    creer_demande(session, fanta, datetime(2026, 1, 5), besoin={"nombre_invites": 30})

    assert repartir_par_tranche_invites(session, etoile.id, PERIODE_JANVIER) == [
        EffectifTranche(tranche="1-50", nombre_demandes=0),
        EffectifTranche(tranche="51-150", nombre_demandes=0),
        EffectifTranche(tranche="151-300", nombre_demandes=0),
        EffectifTranche(tranche="301+", nombre_demandes=0),
    ]
